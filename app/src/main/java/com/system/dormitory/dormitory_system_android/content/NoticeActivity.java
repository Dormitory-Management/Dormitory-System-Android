...
public class NoticeActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Helper_server.post("data/getNoticeComment.php", params, new JsonHttpResponseHandler() {
            @Override

            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {

                try {
                    int sum = Integer.parseInt(response.get("notice_sum").toString());
                    for (int i = 0; i < sum; i++) {
                        DataManager.getInstance().addComment(Integer.parseInt(response.get("notice_sno" + i).toString()),
                                response.get("notice_comment" + i).toString(), response.get("notice_time" + i).toString().substring(5, 16));
                    }
    ...
    void onClick() {
        ...
        if (comment.equals(""))
            ...
            //server connect
            Helper_server.post("data/InsertNoticeComment.php", params, new TextHttpResponseHandler() {
                @Override
                public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                ...
                public void onSuccess(int statusCode, Header[] headers, String responseString) {
                    AlertDialog.Builder alert = new AlertDialog.Builder(NoticeActivity.this);
